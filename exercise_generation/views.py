...
from exercise_generation.services.services import ExerciseGenerationService, NoBacReferenceQuestionsError
...
class ExerciseGenerateAPIView(GenericAPIView):
    ...
    @extend_schema(request=ExerciseGenerationRequestSerializer, responses={201: GeneratedExerciseSerializer(many=True)})
    def post(self, request):
        ...
        try:
            exercises = ExerciseGenerationService().generate(
                axis_id=data["axis_id"], count=data["count"],
                save_to_database=data["save_to_database"], student=request.user,
            )
            return Response({
                "success": True,
            ...
            return Response({"success": False, "error": str(exc), "details": exc.errors}, status=status.HTTP_422_UNPROCESSABLE_ENTITY)
...
class AxisGeneratedExercisesAPIView(GenericAPIView):
    ...
    def get(self, request, axis_id):
        ...
        exercises = GeneratedExercise.objects.filter(
            axis=axis, student=request.user, is_active=True
        ).prefetch_related("alternative_solutions").order_by("-created_at")
        return Response({
...
class ExerciseAlternativeSolutionAPIView(GenericAPIView):
    ...
    @extend_schema(request=AlternativeSolutionRequestSerializer, responses={201: GeneratedExerciseAlternativeSolutionSerializer})
    def post(self, request, exercise_id):
        serializer = self.get_serializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        level = serializer.validated_data.get("simplification_level", "very_simple")
        exercise = get_object_or_404(
            GeneratedExercise.objects.select_related("axis", "axis__chapter", "axis__chapter__subject", "student"),
            id=exercise_id, student=request.user, is_active=True,
        )
        try:
            alternative = AlternativeSolutionService().generate_and_save(
                exercise=exercise, student=request.user, simplification_level=level,
            )
            return Response({
                "success": True,
                "message": "تم إنشاء حل بديل أبسط مع الاحتفاظ بالحل الأول.",
                "exercise": {"id": exercise.id, "title": exercise.title},
                "alternative_solution": GeneratedExerciseAlternativeSolutionSerializer(alternative).data,
            }, status=status.HTTP_201_CREATED)
    ...
    def get(self, request, exercise_id):
        exercise = get_object_or_404(GeneratedExercise, id=exercise_id, student=request.user, is_active=True)
        alternatives = exercise.alternative_solutions.filter(student=request.user).order_by("-created_at")
        return Response({
            "success": True,
            "exercise": {"id": exercise.id, "title": exercise.title},
            "count": alternatives.count(),
            "alternative_solutions": GeneratedExerciseAlternativeSolutionSerializer(alternatives, many=True).data,
        }, status=status.HTTP_200_OK)
